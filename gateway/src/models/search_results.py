...
class SearchResults(Base):
    ...
    query_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("search_history.query_id", ondelete="CASCADE"), nullable=False)
    found_event_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("video_events.event_id", ondelete="CASCADE"), nullable=False)
    ...
    bbox: Mapped[list | None] = mapped_column(JSONB, nullable=True, server_default=text("'[]'::jsonb"))
    hit_type: Mapped[str | None] = mapped_column(String(20), nullable=True)
    is_relevant: Mapped[bool | None] = mapped_column(Boolean, nullable=True, default=None)
